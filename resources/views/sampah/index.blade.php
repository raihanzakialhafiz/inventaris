    ...
    <div class="card">
        <div class="card-b" style="padding:0">
            @if ($rows->count())
                    ...
                    <tbody>
                        @foreach ($rows as $row)
                            ...
                            <tr>
                                ...
                                <td><span class="badge {{ $daysLeft <= 7 ? 'b-danger' : 'b-warn' }}">{{ $daysLeft }}
                                        hari lagi</span></td>
                                <td style="white-space:nowrap">
                                    <div class="act-group">
                                        {{-- Pulihkan --}}
                                        <form method="POST"
                                            action="{{ route('sampah.restore', [$row['slug'], $row['id']]) }}"
                                            style="display:inline"
                                            data-confirm="Pulihkan {{ $row['type'] }} “{{ $row['name'] }}”?"
                                            data-confirm-ok="Pulihkan">
                                            @csrf
                                        ...
                                        {{-- Hapus permanen --}}
                                        <form method="POST"
                                            action="{{ route('sampah.destroy', [$row['slug'], $row['id']]) }}"
                                            style="display:inline"
                ...
                @if ($rows->total())
                    <div class="pg-bar">
                        <span class="pg-info">Menampilkan {{ $rows->firstItem() }}–{{ $rows->lastItem() }} dari
                            {{ $rows->total() }} data</span>
                        {{ $rows->links() }}
                    </div>
                @endif
            @else
