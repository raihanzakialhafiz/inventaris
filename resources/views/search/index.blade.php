...
@section('content')
    <div style="max-width:920px">
        ...
        <form method="GET" action="{{ route('search.index') }}" class="filter-bar" style="margin-bottom:18px">
            ...
            @if ($q !== '')
                <a href="{{ route('search.index') }}" class="btn btn-ghost btn-sm"><x-icon name="x" width="13"
                        height="13" /> Reset</a>
            @endif
        </form>
        ...
        @forelse($groups as $group)
            <div class="card" style="margin-bottom:16px">
                ...
                <div class="card-b" style="padding:0">
                    @foreach ($group['items'] as $item)
                        <a href="{{ $item['url'] }}"
                            ...
                            onmouseover="this.style.background='var(--bg-soft,#f7fafa)'"
                ...
                @if (!empty($group['allUrl']))
                    <a href="{{ $group['allUrl'] }}" class="notif-foot" style="display:block">Lihat semua di
                        {{ $group['label'] }} <x-icon name="arrow-right" width="13" height="13"
            ...
            <div class="card">
                <div class="card-b" style="text-align:center;padding:48px 24px;color:var(--muted,#64748b)">
                    @if ($q === '')
                        ...
                        <b>Mulai mencari</b>
                        <p style="margin-top:4px">Gunakan kolom di atas atau kotak pencarian di header untuk menelusuri
                            seluruh data.</p>
                    @else
                        <div style="font-size:34px;margin-bottom:8px">🗂</div>
                        <b>Tidak ada hasil</b>
                        <p style="margin-top:4px">Tidak ditemukan data yang cocok dengan “{{ $q }}”. Coba kata
                            kunci lain.</p>
                    @endif
                </div>
            </div>
