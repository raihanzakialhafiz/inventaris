...
@section('content')
    <div x-data="{ showModal: false, editData: {} }">
        ...
        <div class="page-head" style="display:flex;align-items:center;justify-content:space-between">
            <div>
                <h2>Satuan Barang</h2>
                <p>Daftar satuan (rim, lusin, buah, …) untuk pilihan pada form barang.</p>
            </div>
            <button class="btn btn-pri" @click="showModal=true; editData={}"><x-icon name="plus" width="15"
                    height="15" /> Tambah Satuan</button>
        </div>
        ...
        <div class="card">
            <div class="card-b" style="padding:0">
                @if ($satuan->count())
                        ...
                        <tbody>
                            @foreach ($satuan as $s)
                                <tr>
                                    ...
                                    <td class="num">
                                        @if ($s->items_using > 0)
                                            <span class="badge b-primary">{{ $s->items_using }} barang</span>
                                        @else
                    ...
                    @if ($satuan->total())
                        <div class="pg-bar">
                            <span class="pg-info">Menampilkan {{ $satuan->firstItem() }}–{{ $satuan->lastItem() }} dari
                                {{ $satuan->total() }} satuan</span>
                            {{ $satuan->links() }}
                        </div>
                    @endif
                @else
                    ...
                    <div class="modal-body">
                        <form :action="editData.id ? '/satuan/' + editData.id : '/satuan'" method="POST">
                            ...
                            <div class="field">
                                ...
                                <input type="text" name="name" :value="editData.name || ''" required maxlength="30"
                                    placeholder="mis. rim, lusin, buah">
                            </div>
                            <div class="field" style="margin:0">
                                <label>Keterangan</label>
                                <input type="text" name="description" :value="editData.description || ''" maxlength="100"
