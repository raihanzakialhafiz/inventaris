...
@section('content')
    <div x-data="supplierData()">
        ...
        <div class="page-head" style="display:flex;align-items:center;justify-content:space-between">
            <div>
                <h2>Supplier</h2>
                <p>Daftar pemasok barang ATK.</p>
            </div>
            <button class="btn btn-pri" @click="openModal({})"><x-icon name="plus" width="15" height="15" /> Tambah
                Supplier</button>
        </div>
        ...
        <form method="GET" x-ref="filterForm" class="filter-bar">
            ...
            <input type="search" name="search" value="{{ request('search') }}"
                placeholder="Cari nama, telepon, atau email…" @input.debounce.400ms="$refs.filterForm.submit()">

            <span class="filter-spacer"></span>

            @if (request()->filled('search'))
                <a href="{{ route('supplier.index') }}" class="btn btn-ghost btn-sm"><x-icon name="x" width="13"
                        height="13" /> Reset</a>
            @endif
                            ...
                            @foreach ($suppliers as $sup)
                                <tr>
                                    ...
                                    <td style="white-space:nowrap">
                                        ...
                                        <form method="POST" action="{{ route('supplier.destroy', $sup) }}"
