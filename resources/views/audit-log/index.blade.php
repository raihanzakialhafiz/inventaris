...
@section('content')
    ...
    <div class="page-head" style="display:flex;align-items:center;justify-content:space-between;gap:10px">
        <div>
            <h2>Audit Log Sistem</h2>
            <p>Rekaman seluruh aktivitas pengguna dalam sistem.</p>
        </div>
        <div style="display:flex;gap:8px;flex:0 0 auto">
            <x-export-ttd :pdf="route('audit-log.export', ['format' => 'pdf'] + request()->query())" :excel="route('audit-log.export', ['format' => 'excel'] + request()->query())" />
        </div>
    </div>

    <form method="GET" x-data x-ref="filterForm" class="filter-bar">
        <input type="search" name="q" value="{{ request('q') }}" placeholder="Cari aktivitas, entitas, IP…"
            @input.debounce.400ms="$refs.filterForm.submit()">

        <div style="min-width:180px">
            <x-searchable-select name="user_id" :options="$users" :selected="request('user_id')" placeholder="Semua Pengguna"
                search-placeholder="Cari pengguna…" :submit-on-change="true" />
        </div>

        <div style="min-width:160px">
            <x-searchable-select name="entity_type" :selected="request('entity_type')" :options="collect($entities)->mapWithKeys(fn($e) => [$e => $e])" placeholder="Semua Entitas"
                search-placeholder="Cari entitas…" :submit-on-change="true" />
        </div>

        <input type="date" name="date" value="{{ request('date') }}" title="Filter tanggal"
            onchange="this.form.submit()" style="width:150px">

        <span class="filter-spacer"></span>

        @if (request()->anyFilled(['q', 'user_id', 'entity_type', 'date']))
            <a href="{{ route('audit-log.index') }}" class="btn btn-ghost btn-sm"><x-icon name="x" width="13"
                    height="13" /> Reset</a>
        @endif
    </form>
                                ...
                                <td>
                                    @php
                                        $actColor = match (true) {
                                            ...
                                            str_contains($log->activity, 'delete') ||
                                                str_contains($log->activity, 'reject')
                                                => 'b-danger',
                                            str_contains($log->activity, 'create') ||
                                                str_contains($log->activity, 'distribusi')
                                                => 'b-ok',
                                            str_contains($log->activity, 'update') ||
                                                str_contains($log->activity, 'approve')
                                                => 'b-warn',
                                            default => 'b-neutral',
                                        };
                                    @endphp
                                    ...
                                    @if ($detail !== '')
                                        <div x-data="{ open: false }">
                                            <button type="button" class="btn btn-ghost btn-sm" @click="open=!open"
                                                x-text="open ? '▼ Tutup' : '▶ Detail'" style="font-size:11px">▶
                                                Detail</button>
