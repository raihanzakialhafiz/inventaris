    ...
    @if ($approved->count() > 0)
        <div class="notice warn mt">
            ...
            <div>
                ...
                <a href="{{ route('distribusi.index') }}" style="margin-left:8px;font-weight:600">Lihat antrean <x-icon
        ...
        <div class="card">
            ...
            <div class="card-b" style="padding:6px 0">
                @forelse($low->merge($out) as $item)
                    ...
                    <div
                        style="display:flex;align-items:center;gap:12px;padding:11px 18px;border-bottom:1px solid var(--line)">
                        ...
                        <div style="flex:1;min-width:0">
                            <div class="t-name" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis">
                                {{ $item->name }}</div>
                            <div class="t-sub">min {{ $item->minimum_stock }} · {{ $item->location ?: 'lokasi ??' }}
                            </div>
                        </div>
                ...
                @forelse($approved->take(6) as $req)
                    <div
                        ...
                        <div style="flex:1;min-width:0">
                            ...
                            <div class="t-sub" style="margin-top:2px">{{ $req->details->count() }} jenis barang ·
                                {{ $req->department->name }}</div>
                        </div>
                        <a href="{{ route('distribusi.index', ['proses' => $req->id]) }}" class="btn btn-pri btn-sm"
                            style="flex:0 0 auto">Proses <x-icon name="arrow-right" width="13" height="13"
                ...
                @if ($approved->count() > 6)
                    <div style="padding:10px 18px;text-align:center;border-top:1px solid var(--line)">
                        <a href="{{ route('distribusi.index') }}" class="btn btn-ghost btn-sm">
                            Lihat semua {{ $approved->count() }} antrean <x-icon name="arrow-right" width="13"
                                height="13" style="vertical-align:-2px" />
                        </a>
                    </div>
                @endif
            </div>
