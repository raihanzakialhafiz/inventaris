        ...
        <div class="card">
            <div class="card-h">
                ...
                <div style="margin-left:auto;display:flex;gap:8px">
                    ...
                    <a href="{{ route('stock-opname.index') }}" class="btn btn-ghost btn-sm"><x-icon name="arrow-left"
                            width="13" height="13" style="vertical-align:-2px" /> Kembali</a>
                </div>
            </div>
            <div class="card-b">
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px 16px;margin-bottom:16px">
                    <div><span class="t-sub">No. Opname</span>
                        <div class="t-name code">{{ $opname->opname_no }}</div>
                    </div>
                    <div><span class="t-sub">Tanggal</span>
                        <div>{{ $opname->date->isoFormat('D MMM Y') }}</div>
                    </div>
                    <div><span class="t-sub">Petugas</span>
                        <div>{{ $opname->createdBy->name ?? '—' }}</div>
                    </div>
                    <div><span class="t-sub">Disesuaikan</span>
                        <div>{{ $opname->adjusted_count }} dari {{ $opname->items_count }} barang</div>
                    </div>
                    @if ($opname->note)
