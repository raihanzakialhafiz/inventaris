...
@section('content')
    <div x-data="{ showModal: false, q: '', match(t) { return !this.q || t.includes(this.q.toLowerCase()); } }">
        ...
        <div class="page-head" style="display:flex;align-items:center;justify-content:space-between">
            <div>
                <h2>Stock Opname</h2>
                <p>Cocokkan stok sistem dengan hitungan fisik, lalu sesuaikan otomatis.</p>
            </div>
            <button class="btn btn-pri" @click="showModal=true"><x-icon name="plus" width="15" height="15" /> Mulai
                Opname</button>
        </div>
        ...
        <div class="card">
            <div class="card-b" style="padding:0">
                @if ($opnames->count())
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th>No. Opname</th>
                                <th>Tanggal</th>
                                <th class="num">Barang Diperiksa</th>
                                <th class="num">Disesuaikan</th>
                                <th>Petugas</th>
                                <th>Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($opnames as $o)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('stock-opname.show', $o) }}" class="btn btn-ghost btn-sm">Detail
                                            <x-icon name="arrow-right" width="13" height="13"
                    ...
                    <div class="pg-bar">
                        <span class="pg-info">Menampilkan {{ $opnames->firstItem() }}–{{ $opnames->lastItem() }} dari
                            {{ $opnames->total() }} sesi</span>
                        {{ $opnames->links() }}
                    </div>
                @else
                    <div class="empty">
            ...
            <div>
                ...
                <div class="modal" style="display:flex;max-width:780px">
                    <div class="modal-head">
                        ...
                        <button type="button" class="close-btn" @click="showModal=false"><x-icon name="x"
                                width="16" height="16" /></button>
                    </div>
                    <div class="modal-body">
                        <p class="t-sub" style="margin-bottom:12px">Isi jumlah <b>fisik</b> hasil hitungan. Stok sistem
                            otomatis disesuaikan bila berbeda.</p>
                        <form method="POST" action="{{ route('stock-opname.store') }}"
                            data-confirm="Simpan stock opname? Stok sistem akan disesuaikan dengan hitungan fisik."
                            data-confirm-ok="Simpan &amp; Sesuaikan">
                            @csrf
                            <div class="field" style="margin:0 0 12px">
                                <label>Catatan</label>
                                <input type="text" name="note" placeholder="Opsional — mis. opname akhir bulan">
                            </div>
                            <input type="search" class="inp" placeholder="Cari barang untuk difilter…" x-model="q"
                                style="margin-bottom:10px">

                            <div class="scroll-box"
                                style="max-height:46vh;overflow-y:auto;border:1px solid var(--line);border-radius:10px">
                                <table>
                                    <thead>
                                        <tr>
                                            <th>Kode</th>
                                    ...
                                    <tbody>
                                        @foreach ($items as $item)
                                            <tr x-show="match(@js(strtolower($item->code . ' ' . $item->name)))">
                                                ...
                                                <td class="t-name">{{ $item->name }} <span class="t-sub">·
                                                        {{ $item->unit }}</span></td>
                                                <td class="num t-sub">{{ $item->stock }}</td>
                                                <td class="num">
                                                    <input type="number" name="counts[{{ $item->id }}]"
                                                        value="{{ $item->stock }}" min="0" max="1000000"
                                                        class="qty-inp" style="width:90px">
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
