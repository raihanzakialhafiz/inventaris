...
@php
  $user = auth()->user();
  // Admin tidak mengajukan permintaan (hanya menyetujui) → tombol disembunyikan.
  $canCreate = $user->role === 'kepala_bidang';
  // $itemOptions (dengan konteks kuota) disiapkan di PermintaanController.
@endphp
...
<div x-data="{
        ...
        detailId: null,
        lines: [{ qty: 1, item_id: '' }],
        openModal() { this.lines = [{ qty: 1, item_id: '' }]; this.showModal = true; },
        ...
        rejectOpen: false, rejectAction: '', rejectNo: '',
        openReject(action, no) { this.rejectAction = action; this.rejectNo = no; this.rejectOpen = true; },
     }">
...
<div class="page-head" style="display:flex;align-items:center;justify-content:space-between">
  <div><h2>Permintaan Barang</h2><p>{{ $viewSubtitle }}</p></div>
  @if($canCreate)
    <button class="btn btn-pri" @click="openModal()"><x-icon name="plus" width="15" height="15" /> Ajukan Permintaan</button>
  @endif
</div>
...
<form method="GET" class="filter-bar">
  ...
  <div style="min-width:170px">
    <x-searchable-select name="status" :selected="request('status')"
                         :options="['pending'=>'Menunggu','disetujui'=>'Disetujui','ditolak'=>'Ditolak','selesai'=>'Selesai','selesai_sebagian'=>'Selesai Sebagian']"
                         placeholder="Semua Status" search-placeholder="Cari status…" :submit-on-change="true" />
  </div>

  @if($user->role !== 'kepala_bidang')
    <div style="min-width:170px">
      <x-searchable-select name="dept" :options="$departments" :selected="request('dept')"
                           placeholder="Semua Bidang" search-placeholder="Cari bidang…" :submit-on-change="true" />
    </div>
  @endif
  ...
  <div class="card-b" style="padding:0">
    @if($permintaan->count())
      <table>
        ...
        <tbody>
          @foreach($permintaan as $req)
            <tr>
              <td><a href="#" @click.prevent="detailId = {{ $req->id }}" class="code" style="cursor:pointer">{{ $req->request_no }}</a></td>
              ...
              <td>
                <div class="act-group">
                  @if($req->status === 'pending' && in_array($user->role, ['kasubag_umum','admin']))
                    ...
                    <form method="POST" action="{{ route('permintaan.approve', $req) }}" style="display:inline"
                          data-confirm="Setujui {{ $req->request_no }} sesuai jumlah yang diminta?"
                          data-confirm-ok="Setujui">
                      @csrf
                      @foreach($req->details as $d)
                        <input type="hidden" name="approved_quantities[{{ $d->id }}]" value="{{ $d->quantity_requested }}">
                      @endforeach
                      <button type="submit" class="icon-act ok" title="Setujui (sesuai jumlah diminta)">
                    ...
                    <button type="button" class="icon-act danger" title="Tolak"
                      ...
                      <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                    </button>
                  @endif
                  {{-- Detail (modal, tetap di halaman ini) --}}
                  <button type="button" class="icon-act" title="Lihat detail" @click="detailId = {{ $req->id }}">
      ...
      <div class="pg-bar">
        ...
        <span class="pg-info">
          Menampilkan {{ $permintaan->firstItem() }}–{{ $permintaan->lastItem() }} dari {{ $permintaan->total() }} permintaan
        </span>
        {{ $permintaan->links() }}
      </div>
    @else
      ...
      <div class="modal" style="display:flex;max-width:720px">
        ...
        <div class="modal-body">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px 16px;margin-bottom:16px">
            <div><span class="t-sub">Pengaju</span><div class="t-name">{{ $req->requester->name }}</div></div>
            <div><span class="t-sub">Bidang</span><div>{{ $req->department->name }}</div></div>
            <div><span class="t-sub">Tanggal</span><div>{{ $req->request_date->isoFormat('D MMM Y') }}</div></div>
            @if($req->approver)
              <div><span class="t-sub">Diproses oleh</span><div>{{ $req->approver->name }} · {{ $req->approved_date?->isoFormat('D MMM Y HH:mm') }}</div></div>
            @endif
          </div>
        ...
        <div class="modal-body">
          ...
          <form method="POST" action="{{ route('permintaan.store') }}"
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
              ...
              <button type="button" class="btn btn-ghost btn-sm" @click="addLine()"><x-icon name="plus" width="15" height="15" /> Tambah Baris</button>
            </div>

            <template x-for="(line, idx) in lines" :key="idx">
              <div style="display:grid;grid-template-columns:1fr 92px auto;gap:10px;align-items:end;margin-bottom:10px">
                <div class="field" style="margin:0">
                  <label x-show="idx===0" style="font-size:12px">Barang <span style="color:var(--danger)">*</span></label>
