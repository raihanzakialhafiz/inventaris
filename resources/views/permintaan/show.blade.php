    ...
    <div x-data="{ showReject: false }">
            ...
            <div class="card-b">
                <div style="display:flex;align-items:flex-start;justify-content:space-between;flex-wrap:wrap;gap:12px">
                    <div>
                        ...
                        <div class="t-sub">Pengaju: <b>{{ $permintaan->requester->name }}</b> ·
                            {{ $permintaan->department->name }}</div>
                        <div class="t-sub">Tanggal: {{ $permintaan->request_date->isoFormat('D MMM Y') }}</div>
                        @if ($permintaan->approver)
                            <div class="t-sub">Diproses oleh: {{ $permintaan->approver->name }} ·
                                {{ $permintaan->approved_date?->isoFormat('D MMM Y HH:mm') }}</div>
                        @endif
                    </div>
                    ...
                    @if ($permintaan->status === 'pending' && in_array($user->role, ['kasubag_umum', 'admin']))
                        <div style="display:flex;gap:8px">
                            <button class="btn btn-danger btn-sm" @click="showReject=true"><x-icon name="x"
                                    width="13" height="13" /> Tolak</button>
                            <button type="submit" form="form-approve" class="btn btn-ok btn-sm"><x-icon name="check"
                                    width="13" height="13" /> Setujui</button>
                        </div>
                    @endif
                </div>
        ...
        @if ($permintaan->status === 'pending' && in_array($user->role, ['kasubag_umum', 'admin']))
            <form id="form-approve" method="POST" action="{{ route('permintaan.approve', $permintaan) }}"
                data-confirm="Setujui permintaan {{ $permintaan->request_no }} sesuai jumlah yang disetujui?"
                                ...
                                <td class="num">
                                    @if ($permintaan->status === 'pending' && in_array($user->role, ['kasubag_umum', 'admin']))
                                        <input type="number" name="approved_quantities[{{ $d->id }}]"
                                            value="{{ $d->quantity_requested }}" min="0"
                                            max="{{ $d->quantity_requested }}" class="qty-inp">
                                    @else
                                        {{ $d->quantity_approved ?? '—' }}
                                    @endif
                                </td>
                    ...
                    <table>
                        <thead>
                            <tr>
                                <th>No. Distribusi</th>
                                <th>Barang</th>
                                <th class="num">Jml Distribusi</th>
                                <th>Tanggal</th>
                            </tr>
                        </thead>
                        <tbody>
                ...
                <div>
                    ...
                    <div class="modal" style="display:flex">
                        <div class="modal-head">
                            ...
                            <button class="close-btn" @click="showReject=false"><x-icon name="x" width="16"
                                    height="16" /></button>
                        </div>
        ...
        <div style="margin-top:16px">
            <a href="{{ route('permintaan.index') }}" class="btn btn-ghost"><x-icon name="arrow-left" width="13"
                    height="13" style="vertical-align:-2px" /> Kembali</a>
            @if ($permintaan->status === 'disetujui' && in_array($user->role, ['petugas_gudang', 'admin']))
                <a href="{{ route('distribusi.index', ['proses' => $permintaan->id]) }}" class="btn btn-pri"
                    style="margin-left:8px">Proses Distribusi <x-icon name="arrow-right" width="14" height="14"
