        ...
        @if ($approved->total() > 0)
            <div class="card" style="margin-bottom:16px">
                ...
                <div class="card-b" style="padding:0">
                    ...
                    @if ($approved->total())
                        <div class="pg-bar">
                            <span class="pg-info">Menampilkan {{ $approved->firstItem() }}–{{ $approved->lastItem() }}
                                dari {{ $approved->total() }} antrean</span>
                            {{ $approved->links() }}
                        </div>
                    @endif
                </div>
                ...
                <div>
                    ...
                    <div class="modal" style="display:flex;max-width:760px">
                        <div class="modal-head">
                            ...
                            <button type="button" class="close-btn" @click="openId = null"><x-icon name="x"
                        ...
                        <div class="modal-body">
                            ...
                            <form id="dist-form-{{ $req->id }}" method="POST"
                                action="{{ route('distribusi.store', $req) }}"
                                ...
                                <table>
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="num">Distribusikan</th>
                                            <th>Alasan (jika dikurangi)</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($req->details as $d)
                                            ...
                                            <tr x-data="{ qty: {{ old("distributions.{$d->id}.qty", min($approvedQty, $d->item->stock)) }}, approved: {{ $approvedQty }} }">
                                                <td>
                                                    ...
                                                    <div class="t-sub">
                                                        {{ $d->item->unit }}{{ $d->item->location ? ' · ' . $d->item->location : '' }}
                                                    </div>
                                                </td>
                                                <td class="num">
                                                    <b
                                                        style="{{ $d->item->stock < $approvedQty ? 'color:var(--danger)' : '' }}">{{ $d->item->stock }}</b>
                                                    @if ($d->item->stock < $approvedQty)
                                                        <span class="badge b-danger"
                                                            style="font-size:10px;margin-left:4px">Kurang</span>
                                                    @endif
                                                </td>
                                                <td class="num"><b>{{ $approvedQty }}</b></td>
                                                <td class="num">
                                                    <input type="number" name="distributions[{{ $d->id }}][qty]"
                                                        x-model.number="qty" min="0" max="{{ $approvedQty }}"
                                                        class="qty-inp" required>
                                                </td>
                                                <td>
                                                    <input type="text"
                                                        name="distributions[{{ $d->id }}][reduction_reason]"
                                                        value="{{ old("distributions.{$d->id}.reduction_reason") }}"
                                                        :required="qty < approved"
                                                        :placeholder="qty < approved ? 'Wajib diisi (jumlah dikurangi)' :
                                                            'Alasan (jika dikurangi)'"
                                                        :style="qty < approved ? 'width:100%;border-color:var(--danger)' :
                                                            'width:100%'">
                                                </td>
                                            </tr>
                                        @endforeach
                        ...
                        <div class="modal-foot">
                            ...
                            <button type="submit" form="dist-form-{{ $req->id }}" class="btn btn-pri"><x-icon
                                    name="check" width="14" height="14" /> Proses Distribusi</button>
                        </div>
                    </div>
                </div>
            </template>
