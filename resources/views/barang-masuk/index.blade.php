...
<div x-data="{
  ...
  <form method="GET" class="filter-bar">
    ...
    <input type="date" name="date_from" value="{{ request('date_from') }}"
           title="Tanggal dari" onchange="this.form.submit()" style="width:150px">
    <span class="t-sub" style="padding:0 2px">—</span>
    <input type="date" name="date_to" value="{{ request('date_to') }}"
           title="Tanggal sampai" onchange="this.form.submit()" style="width:150px">
    ...
    <div class="card-b" style="padding:0">
      @if($stockIns->count())
        ...
        <div class="pg-bar">
          ...
          <span class="pg-info">
            Menampilkan {{ $stockIns->firstItem() }}–{{ $stockIns->lastItem() }} dari {{ $stockIns->total() }} transaksi
          </span>
          {{ $stockIns->links() }}
        </div>
      @else
      ...
      <div class="modal" style="display:flex;max-width:680px">
        ...
        <div class="modal-body">
          <form method="POST" action="{{ route('barang-masuk.store') }}" @submit="saveMasuk($event)">
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;margin:18px 0 10px;padding-top:14px;border-top:1px solid var(--line)">
        ...
        <div class="modal" style="display:flex;max-width:640px">
          ...
          <div class="modal-body">
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px 16px;margin-bottom:16px">
              <div><span class="t-sub">No. Transaksi</span><div class="t-name code">{{ $si->transaction_no }}</div></div>
              <div><span class="t-sub">Tanggal</span><div>{{ $si->date->isoFormat('D MMM Y') }}</div></div>
              <div><span class="t-sub">Supplier</span><div>{{ $si->supplier->name ?? '—' }}</div></div>
              <div><span class="t-sub">Dicatat oleh</span><div>{{ $si->createdBy->name }}</div></div>
              @if($si->note)
                <div style="grid-column:1/-1"><span class="t-sub">Catatan</span><div>{{ $si->note }}</div></div>
              @endif
            </div>
            <table>
              <thead><tr><th>Barang</th><th>Satuan</th><th class="num">Jumlah</th></tr></thead>
              <tbody>
