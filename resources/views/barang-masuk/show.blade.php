    ...
    <div style="max-width:700px">
        <div class="card">
            <div class="card-h">
                ...
                <a href="{{ route('barang-masuk.index') }}" class="btn btn-ghost btn-sm" style="margin-left:auto"><x-icon
                        name="arrow-left" width="13" height="13" style="vertical-align:-2px" /> Kembali</a>
            </div>
            <div class="card-b">
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:18px">
                    <div><span class="t-sub">No. Transaksi</span>
                        <div class="t-name code">{{ $stockIn->transaction_no }}</div>
                    </div>
                    <div><span class="t-sub">Tanggal</span>
                        <div>{{ $stockIn->date->isoFormat('D MMM Y') }}</div>
                    </div>
                    <div><span class="t-sub">Supplier</span>
                        <div>{{ $stockIn->supplier->name ?? '—' }}</div>
                    </div>
                    <div><span class="t-sub">Dicatat oleh</span>
                        <div>{{ $stockIn->createdBy->name }}</div>
                    </div>
                    @if ($stockIn->note)
                        <div style="grid-column:1/-1"><span class="t-sub">Catatan</span>
                            <div>{{ $stockIn->note }}</div>
                        </div>
                    @endif
                </div>
                    ...
                    <tbody>
                        @foreach ($stockIn->details as $d)
                            <tr>
                                ...
                                <td>
                                    <form method="POST"
                                        action="{{ route('barang-masuk.destroy-detail', [$stockIn, $d]) }}"
